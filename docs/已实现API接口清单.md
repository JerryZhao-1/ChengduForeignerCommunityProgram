...
Event 新写入地址使用 `address_zh` / `address_en`，`address_text` 仅保留 legacy read compatibility。公开转换检查双语标题、摘要、正文和地址，草稿仍允许不完整；错误通过 `VALIDATION_ERROR` 返回精确 field path。
...
| `DELETE` | `/admin/discover/posts/:id`                         | 永久删除帖子及关联数据 | `apps/api/src/routes/discover.ts` | `packages/shared/src/contracts/discover.ts` | `packages/shared/src/mock/service.ts` 中 `posts.permanentlyDelete`    |
...
Admin 的 `DELETE /admin/discover/posts/:id` 仅允许管理员调用，会永久级联删除帖子、评论、互动、举报、相关通知、媒体文件资产与存储对象，并用不包含正文或媒体内容的 `permanent_delete_post` 记录替换旧目标审计；用户、地点、活动和标签不受影响。
