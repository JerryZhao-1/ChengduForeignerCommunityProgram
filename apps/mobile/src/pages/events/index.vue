...
const load = async () => {
  ...
  try {
    const [eventResult, registrationResult, favoriteResult] = await Promise.all([
    ...
    registrations.value = Array.isArray(registrationResult.data)
      ...
      : [];
    favoriteEventIds.value = favoriteResult.data.event;
  } catch (err) {
    console.error(err);
...
const eventStatusLabel = (event: Event) => {
  ...
  return signupState.canSignup ? "可报名" : signupState.label;
};

const isFavoriteEvent = (eventId: string) => favoriteEventIds.value.includes(eventId);
...
const toggleFavoriteEvent = async (eventId: string) => {
  ...
  favoriteEventIds.value = result.data.is_favorited
    ? [...favoriteEventIds.value, eventId]
    : favoriteEventIds.value.filter((id) => id !== eventId);
};
        ...
        <view class="content">
          <view class="title-row">
            ...
            <view class="card-actions">
              ...
              <text
                class="favorite-star"
                :class="{ active: isFavoriteEvent(event._id) }"
                @click.stop="toggleFavoriteEvent(event._id)"
              >
                {{ isFavoriteEvent(event._id) ? "★" : "☆" }}
              </text>
            </view>
          </view>
